...
def initialize_chain(resume_pdf_path: str, job_description: str, analysis: str):
    ...
    system_prompt = f"""
You are an AI assistant designed to help a candidate improve their resume to better match job descriptions.
Use the provided resume, job description, and analysis to offer actionable feedback for skill improvements and optimizations.
When necessary, highlight **matching skills**, **missing skills**, and suggest **ways to acquire** the missing skills.
Keep responses focused, practical, and limited to three sentences, prioritizing relevance to the job description.
...
async def upload_resume(
    ...
    # Reset chat messages and session store for this session
    CHAT_MESSAGES = []
    SESSION_STORE[session_id] = ChatMessageHistory()
